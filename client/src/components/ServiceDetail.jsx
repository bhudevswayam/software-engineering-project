...
const ServiceDetail = ({onBookNow, onBack}) => {
  ...
  useEffect(() => {
    const fetchService = async () => {
      try {
        const services = await getRelatedServices(id); // fetch all services
        ...
        const formattedServices = services.services.map((service) => ({
          ...
          images: service.images && service.images.length > 0 ? service.images : serviceDataMock.images,
          specialties: service.specialties && service.specialties.length > 0 ? service.specialties : serviceDataMock.specialties,
          features: service.features && service.features.length > 0 ? service.features : serviceDataMock.features,
          ...
          address: service.addressLine1 && service.city && service.state 
            ? `${service.addressLine1}${service.addressLine2 ? ', ' + service.addressLine2 : ''}, ${service.city}, ${service.state} ${service.zipCode || ''}`
            : null,
          ...
          reviews: service.reviews && service.reviews.length > 0 ? service.reviews : serviceDataMock.reviews,
        }));
        ...
        // Find the specific service by ID or use the first one
        let selectedService;
        if (id) {
          selectedService = formattedServices.find(s => s.id === id);
        }
        
        // If no ID or service not found, use the first service or mock data
        if (!selectedService) {
          selectedService = formattedServices.length > 0 ? formattedServices[0] : serviceDataMock;
        }

        // Find all services from the same business/tenant to use as pricing options
        let pricingOptions = [];
        ...
        // If we have pricing options from the same business, use them
        // Otherwise fall back to the service's own pricing array or mock data
   ...
   { open ? <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          {/* ✅ Select Start Time */}
          <label className="block mb-2">Choose Start Time</label>
          <input
            type="datetime-local"
